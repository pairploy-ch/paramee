...
import { formatCompactBaht, formatBaht } from "@/lib/format";
  ...
  return (
    ...
    >
      <div className="relative">
        ...
        <div className="absolute right-3 top-3 rounded-full bg-maroon-dark/80 px-3 py-1 text-xs font-medium text-cream backdrop-blur">
          {property.type}
        </div>
        <div className="absolute inset-x-0 bottom-0 flex items-center gap-1.5 bg-gradient-to-t from-black/60 to-transparent px-4 py-2.5 text-xs text-cream/90">
          ...
          {property.transit.distanceMeters} ม.
        </div>
      </div>
        ...
        <div className="flex items-baseline justify-between gap-2">
          {property.salePrice ? (
            <p className="font-heading text-lg font-semibold text-gold-dark">
              {formatCompactBaht(property.salePrice)}
            </p>
          ) : (
            <p className="font-heading text-lg font-semibold text-gold-dark">
              {formatBaht(property.rentPrice ?? 0)}
              <span className="text-xs font-normal text-ink/50">/เดือน</span>
            </p>
          )}
        </div>
        ...
        <div className="mt-4 flex items-center gap-4 border-y border-cream-dark py-3 text-xs text-ink/60">
          {property.bedrooms > 0 && (
            <span className="flex items-center gap-1.5">
              <BedDouble className="h-4 w-4" strokeWidth={1.75} aria-hidden /> {property.bedrooms} นอน
            </span>
          )}
          {property.bathrooms > 0 && (
            <span className="flex items-center gap-1.5">
              <Bath className="h-4 w-4" strokeWidth={1.75} aria-hidden /> {property.bathrooms} น้ำ
            </span>
          )}
          <span className="flex items-center gap-1.5">
            <Ruler className="h-4 w-4" strokeWidth={1.75} aria-hidden /> {property.areaSqm} ตร.ม.
          </span>
        </div>
        ...
        <div className="mt-4 flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            ...
            <div className="leading-tight">
              <p className="text-xs font-semibold text-ink/80">ทีมขาย Paramee</p>
              <p className="text-[11px] text-ink/45">Estate Agent</p>
            </div>
          </div>
